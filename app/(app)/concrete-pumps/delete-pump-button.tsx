...
import { deletePump } from "./actions";

export default function DeletePumpButton({
  pumpId,
  pumpName,
  docCount,
}: {
  ...
  function handleClick(e: React.MouseEvent) {
    ...
    const ok = window.confirm(
      `Delete "${pumpName}"${docNote}? This cannot be undone.`,
    );
    if (!ok) return;
    setError(null);
    startTransition(async () => {
      const result = await deletePump(pumpId);
      if (result?.error) {
