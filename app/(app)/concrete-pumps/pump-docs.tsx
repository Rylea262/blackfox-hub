...
export default function PumpDocs({
  ...
  function saveRename(id: string) {
    ...
    startTransition(async () => {
      const result = await renamePumpDoc(id, trimmed);
      if (result?.error) {
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    const path = `${pumpId}/${Date.now()}_${file.name}`;
    ...
    startTransition(async () => {
      const result = await attachPumpDoc(pumpId, file.name, path);
      setIsUploading(false);
      if (result?.error) {
        setError(result.error);
        return;
  ...
  async function handleDownload(doc: PumpDoc) {
    setBusyDownloadId(doc.id);
    setError(null);
    try {
      const url = await getPumpDocUrl(doc.file_url);
  ...
  function handleDelete(doc: PumpDoc) {
    ...
    setError(null);
    startTransition(async () => {
      const result = await deletePumpDoc(doc.id);
      if (result?.error) {
        setError(result.error);
        return;
      }
