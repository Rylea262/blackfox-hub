...
export default function SupplierDocs({
  ...
  function startRename(doc: SupplierDoc) {
    setEditingId(doc.id);
  ...
  function saveRename(id: string) {
    ...
    startTransition(async () => {
      const result = await renameSupplierDoc(id, trimmed);
      if (result?.error) {
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    startTransition(async () => {
      const result = await attachSupplierDoc(supplierId, file.name, path);
      setIsUploading(false);
      if (result?.error) {
        setError(result.error);
        return;
  ...
  async function handleDownload(doc: SupplierDoc) {
    setBusyDownloadId(doc.id);
    setError(null);
    try {
      const url = await getSupplierDocUrl(doc.file_url);
  ...
  function handleDelete(doc: SupplierDoc) {
    ...
    setError(null);
    startTransition(async () => {
      const result = await deleteSupplierDoc(doc.id);
      if (result?.error) {
        setError(result.error);
        return;
      }
