...
export default function SubcontractorDocs({
  subId,
  docs,
}: {
  subId: string;
  ...
  function startRename(doc: SubDoc) {
    setEditingId(doc.id);
  ...
  function saveRename(id: string) {
    ...
    startTransition(async () => {
      const result = await renameSubDoc(id, trimmed);
      if (result?.error) {
  ...
  async function handleUpload(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    startTransition(async () => {
      const result = await attachSubDoc(subId, file.name, path);
      setIsUploading(false);
      if (result?.error) {
        setError(result.error);
        return;
  ...
  async function handleDownload(doc: SubDoc) {
    setBusyDownloadId(doc.id);
    setError(null);
    try {
      const url = await getSubDocUrl(doc.file_url);
  ...
  function handleDelete(doc: SubDoc) {
    ...
    setError(null);
    startTransition(async () => {
      const result = await deleteSubDoc(doc.id);
      if (result?.error) {
        setError(result.error);
        return;
      }
  ...
  return (
    <div className="mt-3 border-t border-neutral-200 pt-3">
      <div className="flex items-center justify-between gap-3">
        <h3 className="text-sm font-medium text-neutral-700">Documents</h3>
        <label
